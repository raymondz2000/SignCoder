...
/* ------------------------- dealingwith alloc error ------------------------ */

/* --------------------------------- function and constant prototype -------------------------------- */

void findhorizontal(unsigned char vertical[],unsigned char horizontal[]);
...
/* ------------------------- find horizontal function ------------------------ */
 ...
 void findhorizontal(unsigned char vertical[],unsigned char horizontal[]){
   ...
   // unsigned char *horizontal=(unsigned char *)calloc(8,sizeof(unsigned char));
...
int main() {
    ...
    unsigned char *message=(unsigned char *)calloc((12*(counter+numChars)),sizeof(char)); 
   ...
   // message=calloc((12*(counter+numChars)),sizeof(char));
    ...
    if(orientation=='V'||orientation=='v'){
        ...
        while(string[j]!='\0'){
           findhorizontal(getvertical(string[j]),array);

            for(int i=0; i<8; ++i){

               message[count] = array[i];    
